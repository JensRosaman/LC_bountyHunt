...
public class Matrix
{
    ...
    public Matrix(string mode = "csv", int randSize = 10, string path = @"C:\Users\olive\Desktop\")
    {
        this.SavePath = path;
        this.SavePath = path;
        if (mode == "csv")
        {
            ...
            string csvPath = Path.Combine(path,"matrix.csv");
            if (File.Exists(csvPath))
            {
                // Read the CSV file
                
                using (var csvReader = new StringReader(File.ReadAllText(csvPath)))
                using (var parser = new CsvTextFieldParser(csvReader))
                {
                    ...
                    {
                        // Read current line fields, assuming each field is a string
                        var fields = parser.ReadFields();

                        // Update the number of columns based on the row with the most fields
                ...
                using (CsvTextFieldParser parser = new CsvTextFieldParser(csvPath, Encoding.UTF8))
                {

                    // Reset the parser to start reading from the beginning again
                    
                    parser.SetDelimiter(';');

                    // Create a matrix (2D array) to store the data as doubles
                    ...
                    while (!parser.EndOfData)
                    {
                        ...
                        {
                            if (double.TryParse(fields[col], out double value))
                            {
                                matrix[row, col] = value;
                            }
                            else
                            {
                                // Handle parsing errors or invalid data here
                            }
                        }
        ...
        { 
            ...
            int size;
            size = (randSize < 1) ? rand.Next(4, 10) : randSize;
    ...
    {
        save_matrix(this.matrix);
    }
    public void save_matrix(double[,] matrix, string fileName = "matrix", string sep = "\t")
    {
        int rows = matrix.GetLength(0);
        int cols = matrix.GetLength(1);
        // skapar en lokaliserad fil ställe
        string filePath = this.SavePath + fileName + ".txt";
        using (StreamWriter writer = new StreamWriter(filePath))
        {
            ...
            for (int i = 0; i < rows; i++)
            {
                for (int j = 0; j < cols; j++)
                {
                    ...
                    writer.Write(sep); // Use "\t" for tab separation or ;
                }
    ...
    public double[,] get_inner(double[,] matrix, int rowIndexToIgnore, int colIndexToIgnore)
    {
        // gets the inner matrix by ignoring a collum and a row
        {
            ...
            double[,] innerMatrix = new double[numRows - 1, numCols - 1];

            for (int i = 0, innerI = 0; i < numRows; i++) 
            {
                if (i == rowIndexToIgnore)
                {
                    continue; // Skip the row 
                }

                for (int j = 0, innerJ = 0; j < numCols; j++)
                {
                    if (j == colIndexToIgnore)
                    {
                        continue; // Skip the column to ignore
                    }
                    innerMatrix[innerI, innerJ] = matrix[i, j];
                    innerJ++;
                }
                innerI++;
            }
    ...
    double det_2x2(double[,] matris)
    {
        // gets the 2x2 det
        double posVal = matris[0, 0] * matris[1, 1];
        double negVal = matris[1, 0] * matris[0, 1];
        return posVal - negVal;
    }

    public double calc_det()
    {
        return calc_det(this.matrix);
    }
    private double calc_det(double[,] matrix)
    {
        ...
        {
            double subMatrixDeterminant = calc_det(get_inner(matrix, 0, j));
            det += (j % 2 == 0 ? 1 : -1) * matrix[0, j] * subMatrixDeterminant; // Varannanat tecken är -
        }
        return det;
    }
}
